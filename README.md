...
准备工具：xshell（远程连接服务器）、postman（测试接口）、宝塔（建站工具，安装在服务器上）

1. 买一个云服务器
2. 通过 xshell 远程连接服务器
3. 安装宝塔
4. 访问服务器上的 8888 端口（宝塔服务端口），要设置服务器的安全组
5. 快速安装
6. 新建数据库，导入 sql 数据
7. 开启网站服务器端口 3001，数据库服务端口 3306；并且阿里云上要在安全组上开通对应的端口
8. 导入文件（先压缩，再导入，在解压），更改 default 配置文件路径（注意：一定要认真填写）、app.js 中的端口号
9. 在软件商店中安装 pm2 管理器
10. 打开 pm2，添加项目，选择要启动的文件目录、文件名，输入项目名，点击提交，就可以启动这个服务(也可以命令行形式开启)
11. 打开 postman，测试接口是否可用
12. 打开前端项目，更改默认配置路径 this.baseURL = 'http://47.100.203.172:3001/api/private/v1/'
13. 重新打包一下，打包后，将 dist 文件压缩，上传到服务器，解压

14. 宝塔上添加一个站点，在文件中找到对应的http目录，先将http中文件全删除，然后上传dist压缩包，解压，将dist内部文件全部剪切到http根目录中
15. 开放阿里云服务器80端口，因为上面站点默认80端口，然后就可以访问了